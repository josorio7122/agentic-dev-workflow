...
function formatError(r: SingleResult): string {
	...
	if (r.stopReason && r.stopReason !== "end_turn") {
		parts.push(`stop: ${r.stopReason}`);
	}
	if (r.exitCode !== 0) {
		parts.push(`exit ${r.exitCode}`);
	}
	if (r.errorMessage) {
		parts.push(r.errorMessage.trim());
	}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description: [
			...
			'Default agent scope is "user" (from ~/.pi/agent/extensions/subagent/agents).',
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (params.tasks && params.tasks.length > 0) {
				...
				const summaries = results.map((r) => {
					...
					if (isError) {
						return `[${r.agent}] FAILED: ${formatError(r)}`;
					}
