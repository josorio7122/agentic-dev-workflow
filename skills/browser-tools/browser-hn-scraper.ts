 ...
 * Usage: browser-hn-scraper.ts [--limit <number>]
 */
 ...
 * @param limit - Maximum number of submissions to return (default: 30)
 * @returns Array of submission objects
 */
export async function scrapeHackerNews(limit = 30): Promise<Submission[]> {
	...
	try {
		...
		$(".athing").each((_index, element) => {
			if (submissions.length >= limit) return false;
		...
		console.error("Error scraping Hacker News:", (error as Error).message);
		throw error;
	}
}
